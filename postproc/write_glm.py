...
for network_id, network in networks.iterrows():
	...
	if not settings["GLM_NOMINAL_VOLTAGE"]:
		if settings["GLM_INCLUDE"]: # cannot verify setting in GLM_INCLUDE until run in gridlabd
			...
			raise Exception("GLM_NOMINAL_VOLTAGE must be defined in either 'config.csv' or the GLM_INCLUDE file")
...
if network_count == 0:
	print("*** WARNING ***")
	print(f"  The network pattern '{settings['GLM_NETWORK_MATCHES']}' did not match any networks in the database")
